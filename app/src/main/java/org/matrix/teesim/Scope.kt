 ...
 * resolution, the raw-`uid:N` syntax, the auto-include expansion and the low-uid warnings live
 * once, not copied three ways with drift between them.
 *
 * An `apps[]` entry is one of these shapes (see [parse]): a plain package name (the app as
 * installed for the primary user), a `pkg@N` token naming the same app inside Android user N — a
 * work profile or a secondary user, whose clone of an app is a different caller uid entirely — an
 * advanced `uid:N` token that targets a caller uid directly, or something malformed. A package name
 * is kept on the wire verbatim even when the app is not installed yet, because a later install
 * still name-matches; a uid is only ever pushed when it actually resolves (never -1).
 *
...
object Scope {
     ...
     * package-shaped (installed or not) and null for a `uid:N` token or a malformed entry — which
     * is exactly the "is this a wire package name?" test [ProfileScope.packageNames] uses. [userId]
     * is the Android user the entry names: 0 for a plain package name, N for `pkg@N`, and (for a
     * raw uid token) the user that uid already encodes.
     */
    data class Explicit(
        val entry: String,
        val kind: Kind,
    ...
    data class ProfileScope(
        ...
        val uidPackages:
            Map<Int, String>, // wire uidPackages[]: the package name behind a resolved uid
        val lowUids:
            Set<Int>, // effective uids whose app id is below FIRST_APP_UID, for the warning
    )
    ...
    fun parse(entry: String): Explicit {
        ...
        if (e.startsWith("uid:")) {
            ...
            val n =
                if (digits.isNotEmpty() && digits.all { it.isDigit() }) digits.toIntOrNull()
                else null
            return if (n != null) Explicit(e, Kind.RAW_UID, n, null, Packages.userIdOf(n))
            else Explicit(e, Kind.INVALID, -1, null, 0)
        }
        // pkg@N splits into the package and the Android user it names; a bare package name is user
        // 0. Everything after the first '@' must be digits, so an '@' inside the package part
        // (never legal in a package name anyway) still falls through to INVALID below.
        val at = e.indexOf('@')
        val name = if (at < 0) e else e.substring(0, at)
        val userDigits = if (at < 0) "" else e.substring(at + 1)
        val userId =
            if (at < 0) 0
            else if (userDigits.isNotEmpty() && userDigits.all { it.isDigit() })
                userDigits.toIntOrNull() ?: -1
            else -1
        if (
            userId >= 0 &&
                name.isNotEmpty() &&
            ...
            // — the name still rides the wire so a later install name-matches, yet contributes no
            // uid until it resolves.
     ...
     * read-only aggregators below ([allTargetUids], [uidToProfile], [uidToPackage]) — some driven
     * by a WebUI endpoint that may be polled — re-derive the same uids and would otherwise re-emit
     * every line (and re-warn about every privileged uid) on each call, so they resolve quietly.
     */
    fun resolve(
        ...
        // on the logging (push) path — the point of it is the hint below, which the quiet
        // aggregators do not emit anyway.
        ...
        for (x in explicit) {
            ...
            when (x.kind) {
                Kind.PACKAGE -> {
                    uids.add(x.uid)
                    // The uid -> package pairing the legacy keystore1 path needs: downstream of
                    // that hook the caller's attestation application id is gone, so it rebuilds one
                    // from this name. It must be carried explicitly, never inferred from array
        ...
        // Android user) is folded in ONLY when NONE of its package names was present at the
        // baseline — i.e. the app was installed AFTER TEESimulator first ran — and no OTHER profile
        // claims it and this profile does not already name it. The baseline (known_packages.json,
        // seeded once) is what makes this "new apps only": every app that already existed is by
        // definition in the baseline and never auto-added. The baseline holds bare package names,
        // so a work profile's clone of an app that user 0 already had counts as pre-existing and is
        // NOT auto-added — the safe direction for a rule whose failure mode is applying a keybox
        // where nobody asked.
        ...
        if (profile.autoIncludeNewApps) {
            ...
            // auto-include would fold in the whole device and apply the keybox everywhere. Fail
            // safe — add nothing until a real baseline exists (the seed retries on a later
            // resolve).
            if (baseline.isEmpty()) {
                if (!quiet)
                    SystemLogger.warning(
                ...
                for (app in Packages.installedAppsByUid()) {
                    ...
                    // Every member package known at baseline => pre-existing app, skip. Any member
                    // absent from the baseline => installed after it, so this uid is genuinely new
                    // and auto-includes.
    ...
    /**
     * The resolved truth of the last config push — what the native lib was actually handed.
     * Published once per push by [Resolver] and read by everything else, so a profile is resolved
     * in exactly one place. That matters for cost as much as for coherence: resolving a profile
     * that auto-includes enumerates every installed app through PackageManager, and the read-only
     * aggregators below used to pay that on every call, on whatever thread called them.
     */
    data class Resolved(
    ...
    /**
     * Publish [scopes] as the resolved truth of the push carrying [epoch]. One immutable object
     * built and stored with a single volatile write, never mutated in place, so a reader on a
     * KeyAdmin connection thread sees either the whole old snapshot or the whole new one and never
     * a torn mix.
     */
     ...
     * through to [baselineKnownPackages]. That function seeds and freezes the baseline on first
     * call, and freezing it from a request thread would fix "which apps count as new" at an
     * arbitrary moment — an HTTP GET would silently decide what auto-include means for the rest of
     * the install.
     */
    private fun baselineSeeded(): Boolean = baselineCache?.isNotEmpty() == true

    /**
     * The scopes to answer read-only questions from. Normally the last published snapshot; a
     * [force] caller (a user-driven refresh) pays for a fresh live resolve instead, and so does
     * anyone asking before the first push has happened — KeyAdmin starts serving before the
     * daemon's first resolve.
     */
    private fun scopesFor(config: ConfigStore.Config, force: Boolean): List<ProfileScope> {
        ...
        return config.profiles.map { p ->
            resolve(p, config.profiles.filter { it.id != p.id }, quiet = true)
        }
    }

    // The baseline set, cached after the first read/seed. The file is written exactly once (when
    // absent) and never rewritten, so an in-memory cache is safe for the daemon's lifetime.
     ...
     * [Const.knownPackagesFile]. Seeds the file — with every currently-installed package name — the
     * first time it is absent, then persists it, so the "future-installs only" auto-include has a
     * fixed frame of reference. Call once at daemon start (before the first resolve) to freeze the
     * baseline at a known moment; later calls reuse the cache / the persisted file.
     *
     ...
     * baseline would make `packages.all { it in baseline }` false for every app and auto-include
     * the entire device, the opposite of "future-installs only". An empty seed therefore returns
     * empty for this call WITHOUT writing the file, so the very next call retries and seeds
     * properly once PackageManager answers. A genuinely-present baseline file (even if it read back
     * empty because the device really had nothing) is honoured; only the seed path guards against
     * the transient case.
     *
        ...
        if (f.exists()) {
            ...
            val set =
                try {
                    ...
                    // A corrupt baseline file is a hard problem retrying can't fix, but caching an
                    // empty set here would auto-include everything — so return empty WITHOUT
                    // caching and let a later call try again (a user can also delete the file to
                    // force a fresh seed).
                    SystemLogger.warning(
                        "Scope: known_packages.json unreadable; not caching empty baseline",
                        e,
                    )
                    return emptySet()
                }
            // An EMPTY existing baseline (truncated write, hand-edit) is never cached and falls
            // through to a reseed below — auto-include is disabled (resolve() guards on empty)
            // until a real set is written, rather than freezing "everything is new".
            if (set.isNotEmpty() && version >= BASELINE_VERSION) {
                baselineCache = set
                return set
            }
            if (set.isNotEmpty()) {
                // Pre-multi-user baseline: fold in the apps the v1 file COULD NOT SEE, and only
                // those — the packages that exist in some other user and not in the primary one.
                //
                // Emphatically NOT today's whole enumeration. The baseline's meaning is "what
                // existed when TEESimulator first ran", and user 0's part of it is already recorded
                // accurately;
                // merging user 0's CURRENT list would advance that frame of reference to now,
                // marking every app installed since the seed as pre-existing and silently dropping
                // it out of auto-include — on a single-user device too, where this migration has
                // nothing to add. A package is excluded when the primary user has it for the same
                // reason: the baseline is keyed by bare name, so folding in a work profile's copy
                // of an app user 0 also installed after the seed would lose that app just the same.
                //
                // An enumeration that comes back empty ENTIRELY is the transient failure the seed
                // path guards against, so the old set is returned UNCACHED and the top-up retries
                // on the next call. Nothing is auto-included meanwhile: resolve() walks the very
                // enumeration that just failed. An empty result with a non-empty enumeration is not
                // a failure — it is a device with nothing outside user 0, which upgrades to v2
                // adding nothing.
            ...
            SystemLogger.warning(
                "Scope: known_packages.json is empty; reseeding from a live enumeration"
            )
        }
        // No baseline yet: seed from a live enumeration. If it comes back empty, treat it as a
        // transient failure — do not write or cache — so the baseline is only ever frozen from a
        // real package set.
        val seeded = Packages.allInstalledPackageNames()
        if (seeded.isEmpty()) {
            SystemLogger.warning(
                "Scope: package enumeration empty; deferring known_packages.json seed (will retry)"
            )
            return emptySet()
        }
        SystemLogger.info(
            "Scope: seeding known_packages.json baseline with ${seeded.size} package(s)"
        )
     ...
     * into the "empty baseline" case above. A failure is logged, not thrown: the in-memory set
     * still holds for this run, and the next start seeds again.
     */
    private fun writeBaseline(f: File, packages: Set<String>) {
        try {
            ...
            SystemLogger.info(
                "Scope: wrote known_packages.json (v$BASELINE_VERSION, ${packages.size} package(s))"
            )
        } catch (e: Exception) {
    ...
    /**
     * The set of explicit caller uids (installed packages + `uid:N` tokens) named across
     * [profiles].
     */
     ...
     * collisions a validation error, so last-writer-wins here is only ever exercised by
     * auto-include overlap, which validation already forbids (at most one auto profile).
     */
    ...
    /**
     * uid -> a representative app name for the WebUI's stored-keys view. A resolved package entry
     * maps to the entry as written — so a secondary user's app keeps its `pkg@user` spelling and
     * cannot be mistaken for the primary user's copy; a raw or auto-included uid has no package, so
     * it maps to its `uid:N` token so the UI still shows something recognisable. Package entries
     * win over tokens on the same uid.
     */
    fun uidToPackage(config: ConfigStore.Config, force: Boolean = false): Map<Int, String> {
        val map = HashMap<Int, String>()
        for (scope in scopesFor(config, force)) {
            for (x in scope.explicit) if (x.kind == Kind.PACKAGE && x.pkg != null)
                map[x.uid] = x.entry
