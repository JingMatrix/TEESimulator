 ...
 * child: an in-process native reader ([nativeRun], in libteesim_logcat.so) reads logd directly
 * through liblog's reader API, filters in C to the lines we keep — every `TEESimulator` line (the
 * daemon, the TA and both native interceptors log under that tag), `AndroidRuntime` and fatal-level
 * lines (Java/native crash reports), and, once the interceptor is injected, every line from the
 * target keystore process ([targetPid]) — and serves them two ways: a bounded ring the WebUI polls
 * incrementally ([snapshot]), and rotating files under [Const.logDir] so a crash's context survives
 * a restart.
 *
 * The old `logcat`-subprocess reader pulled the WHOLE device stream through a pipe and
 * regex-filtered it in the JVM, so any flood (e.g. the KeyAdmin accept loop in #265) was carried at
...
object LogTail {
    ...
    // Field separator the native side packs each polled line with:
    // "seq<SEP>level<SEP>tag<SEP>text". Neither a tag nor a log message contains U+0001, so the
    ...
    /**
     * The injected keystore/keystore2 pid, set by [Injector]; -1 when none is live. Forwarded to
     * the native reader so its lines are kept even though they carry neither our tag nor a fatal
     * level.
     */
    @Volatile
    var targetPid: Int = -1
    ...
    /**
     * Load libteesim_logcat.so from [nativeLib] and start the reader on its own daemon thread.
     * Called once, from [App], with the module's per-ABI library path. If the library is missing or
     * fails to load the Logs panel simply stays empty; the daemon is otherwise unaffected.
     */
    fun start(nativeLib: File) {
        ...
        try {
            ...
            SystemLogger.error(
                "LogTail: cannot load ${nativeLib.absolutePath}; Logs panel disabled",
                e,
            )
            return
        }
        ...
        // JNI name was renamed) — an uncaught throwable on this thread would kill the whole
        // process, so it is contained here and merely disables the Logs panel.
        Thread(
                {
                    try {
                        ...
                        SystemLogger.error(
                            "LogTail: native log reader thread died; Logs panel disabled",
                            e,
                        )
                    }
                },
    ...
    /**
     * Lines with seq greater than [after], up to [max], plus the cursor to poll with next. Empty
     * (and the cursor unchanged) until the native reader is loaded.
     */
